...
rectangle(hAx, 'Position', [xpeak, ypeak, size(B,2), size(B,1)])

% get 1nd match area
match_1 = A_og(ypeak:size(B,1)+ypeak,xpeak:size(B,2)+xpeak);
imshow(match_1)
...
match_2 = A_og(ypeak:size(B,1)+ypeak,xpeak:size(B,2)+xpeak);
imshow(match_2)
